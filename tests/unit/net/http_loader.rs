...
fn  test_redirect_from_x_to_y_provides_y_cookies_from_y() {
    let handler = move |request: HyperRequest, mut response: HyperResponse| {
        ...
        if path == "/com/" {
            ...
            response.headers_mut().set(Location("http://mozilla.org".to_owned()));
            *response.status_mut() = StatusCode::MovedPermanently;
            response.send(b"").unwrap();
        } else if path == "/org/" {
            assert_eq!(request.headers.get(),
    ...
    let url_x = ServoUrl::parse("http://mozilla.com/com").unwrap();
    let url_y = ServoUrl::parse("http://mozilla.org/org").unwrap();
